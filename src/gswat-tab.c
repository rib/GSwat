...
#define GSWAT_TAB_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), GSWAT_TYPE_TAB, GSwatTabPrivate))

#define GSWAT_TAB_KEY "GSWAT_TAB_KEY"

struct _GSwatTabPrivate
{
	GSwatTabState	        state;
...
G_DEFINE_TYPE(GSwatTab, gswat_tab, GTK_TYPE_VBOX)
...
//static gboolean gswat_tab_auto_save (GSwatTab *tab);

#if 0
static void
install_auto_save_timeout (GSwatTab *tab)
{
	gint timeout;
	...
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_LOADING);
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_SAVING);
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_REVERTING);
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_LOADING_ERROR);
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_SAVING_ERROR);
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_SAVING_ERROR);
	g_return_if_fail (tab->priv->state != GSWAT_TAB_STATE_REVERTING_ERROR);
	
	/* Add a new timeout */
	timeout = g_timeout_add (tab->priv->auto_save_interval * 1000 * 60,
				 (GSourceFunc) gswat_tab_auto_save,
				 tab);
...
#if 0
static gboolean
install_auto_save_timeout_if_needed (GSwatTab *tab)
{
	GeditDocument *doc;
	...
	g_return_val_if_fail ((tab->priv->state == GSWAT_TAB_STATE_NORMAL) ||
			      (tab->priv->state == GSWAT_TAB_STATE_SHOWING_PRINT_PREVIEW) ||
			      (tab->priv->state == GSWAT_TAB_STATE_CLOSING), FALSE);

	if (tab->priv->state == GSWAT_TAB_STATE_CLOSING)
		return FALSE;

	doc = gswat_tab_get_document (tab);
...
#if 0
static void
remove_auto_save_timeout (GSwatTab *tab)
{
	gedit_debug (DEBUG_TAB);
...
static void
gswat_tab_get_property (GObject    *object,
		        guint       prop_id,
		        GValue     *value,
		        GParamSpec *pspec)
{
	GSwatTab *tab = GSWAT_TAB (object);

	switch (prop_id)
	{
		case PROP_NAME:
			g_value_take_string (value,
					     _gswat_tab_get_name (tab));
			break;
		case PROP_STATE:
			g_value_set_int (value,
					 gswat_tab_get_state (tab));
			break;
#if 0
		case PROP_AUTO_SAVE:
			g_value_set_boolean (value,
					     gswat_tab_get_auto_save_enabled (tab));
			break;
		case PROP_AUTO_SAVE_INTERVAL:
			g_value_set_int (value,
					 gswat_tab_get_auto_save_interval (tab));
			break;
#endif
		default:
...
static void
gswat_tab_set_property (GObject      *object,
		        guint         prop_id,
		        const GValue *value,
		        GParamSpec   *pspec)
{
...
#if 0
		case PROP_AUTO_SAVE:
			gswat_tab_set_auto_save_enabled (tab,
							 g_value_get_boolean (value));
			break;
		case PROP_AUTO_SAVE_INTERVAL:
			gswat_tab_set_auto_save_interval (tab,
...
static void
gswat_tab_finalize (GObject *object)
{
	GSwatTab *tab = GSWAT_TAB (object);
	...
	G_OBJECT_CLASS (gswat_tab_parent_class)->finalize (object);
}

static void 
gswat_tab_class_init (GSwatTabClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = gswat_tab_finalize;
	object_class->get_property = gswat_tab_get_property;
	object_class->set_property = gswat_tab_set_property;
	...
	g_object_class_install_property (object_class,
					 ...
					 g_param_spec_int ("state",
							   "State",
							   "The tab's state",
							   0, /* GSWAT_TAB_STATE_NORMAL */
							   GSWAT_TAB_NUM_OF_STATES - 1,
							   0, /* GSWAT_TAB_STATE_NORMAL */
							   G_PARAM_READABLE));							      	
	...
	g_type_class_add_private (object_class, sizeof (GSwatTabPrivate));
}


GSwatTabState
gswat_tab_get_state (GSwatTab *tab)
{
	g_return_val_if_fail (GSWAT_IS_TAB (tab), GSWAT_TAB_STATE_NORMAL);
			       ...
			       GSwatTabState  state)
{
	...
	if ((state == GSWAT_TAB_STATE_LOADING)          ||
	    ...
	    (state == GSWAT_TAB_STATE_PRINTING)         ||
	    (state == GSWAT_TAB_STATE_PRINT_PREVIEWING) ||
...
static void
set_view_properties_according_to_state (GSwatTab      *tab,
					GSwatTabState  state)
{
#if 0
	gboolean val;

	val = ((state == GSWAT_TAB_STATE_NORMAL) &&
	       (tab->priv->print_preview == NULL) &&
	       !tab->priv->not_editable);
	gtk_text_view_set_editable (GTK_TEXT_VIEW (tab->priv->view), val);

	val = ((state != GSWAT_TAB_STATE_LOADING) &&
	       (state != GSWAT_TAB_STATE_CLOSING));
...
static void
gswat_tab_set_state (GSwatTab      *tab,
		     GSwatTabState  state)
{
	g_return_if_fail (GSWAT_IS_TAB (tab));
	g_return_if_fail ((state >= 0) && (state < GSWAT_TAB_NUM_OF_STATES));
	...
	if ((state == GSWAT_TAB_STATE_LOADING_ERROR) || // FIXME: add other states if needed
...
remove_tab (GSwatTab *tab)
{
	...
	notebook = GSWAT_NOTEBOOK (gtk_widget_get_parent (GTK_WIDGET (tab)));

	gswat_notebook_remove_tab (notebook, tab);
}
#endif
						 ...
						 GSwatTab         *tab)
{
	...
	if (response_id == GTK_RESPONSE_OK)
	{
		...
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_LOADING);
                ...
                GSwatTab         *tab)
{
	...
	g_object_ref (tab);
	gedit_document_load_cancel (gswat_tab_get_document (tab));
	g_object_unref (tab);	
}
						     ...
						     GSwatTab         *tab)
{
	...
	gswat_tab_set_state (tab,
			     GSWAT_TAB_STATE_NORMAL);
...
static void
show_loading_message_area (GSwatTab *tab)
{
	GtkWidget *area;
	...
	if (tab->priv->state == GSWAT_TAB_STATE_REVERTING)
	{
		if (dirname != NULL)
		{
...
#if 0
static void
show_saving_message_area (GSwatTab *tab)
{
	GtkWidget *area;
	GeditDocument *doc = NULL;
	gchar *short_name;
...
static void
message_area_set_progress (GSwatTab	    *tab,
		  ...
		  GSwatTab         *tab)
{
	...
	g_return_if_fail ((tab->priv->state == GSWAT_TAB_STATE_LOADING) ||
		 	  (tab->priv->state == GSWAT_TAB_STATE_REVERTING));
		 ...
		 GSwatTab      *tab)
{
	...
	if (error != NULL)
	{
		if (tab->priv->state == GSWAT_TAB_STATE_LOADING)
			gswat_tab_set_state (tab, GSWAT_TAB_STATE_LOADING_ERROR);
		else
			gswat_tab_set_state (tab, GSWAT_TAB_STATE_REVERTING_ERROR);
		...
		if (error->domain == GEDIT_DOCUMENT_ERROR)
		{
			if (error->code == GNOME_VFS_ERROR_CANCELLED)
			{
				 ...
				 */
                //FIXME send a signal up to our parent so that
                //it knows things have gone wrong
			...
			else
			{
				...
				if (tab->priv->state == GSWAT_TAB_STATE_LOADING_ERROR)
				{
		...
		else
		{
			...
			// TODO: different error messages if tab->priv->state == GSWAT_TAB_STATE_REVERTING?
	...
	else
	{
        ...
        /* XXX need to re-implement check for re-opening
		...
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_NORMAL);
		
        /* XXX files are always read only */
		//install_auto_save_timeout_if_needed (tab);
	}
						  ...
						  GSwatTab         *tab)
{
	...
	if (tab->priv->print_preview != NULL)
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_SHOWING_PRINT_PREVIEW);
	else
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_NORMAL);
				       ...
				       GSwatTab         *tab)
{
	if (response_id == GTK_RESPONSE_YES)
	{
		...
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_SAVING);
		...
		GSwatTab      *tab)
{
	...
	if (error != NULL)
	{
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_SAVING_ERROR);
...
static void
gswat_tab_init(GSwatTab *tab)
{
	GtkWidget *sw;
	GeditDocument *doc;

	tab->priv = GSWAT_TAB_GET_PRIVATE (tab);

	tab->priv->state = GSWAT_TAB_STATE_NORMAL;
	...
	g_object_set_data (G_OBJECT (doc), GSWAT_TAB_KEY, tab);

	tab->priv->view = gswat_view_new(doc);
	g_object_unref (doc);
	gtk_widget_show (tab->priv->view);
	g_object_set_data (G_OBJECT (tab->priv->view), GSWAT_TAB_KEY, tab);
...
_gswat_tab_new (void)
{
	return GTK_WIDGET (g_object_new (GSWAT_TYPE_TAB, NULL));
}

/* Whether create is TRUE, creates a new empty document if location does 
   not refer to an existing file */
GtkWidget *
_gswat_tab_new_from_uri (const gchar         *uri,
			 const GeditEncoding *encoding,
			 gint                 line_pos,			
			 gboolean             create)
{
...
GSwatView *
gswat_tab_get_view (GSwatTab *tab)
{
	return GSWAT_VIEW (tab->priv->view);
}

/* This is only an helper function */
GeditDocument *
gswat_tab_get_document (GSwatTab *tab)
{
...
gchar *
_gswat_tab_get_name (GSwatTab *tab)
{
	GeditDocument *doc;
	gchar *name;
	gchar *docname;
	...
	g_return_val_if_fail (GSWAT_IS_TAB (tab), NULL);
...
gchar *
_gswat_tab_get_tooltips	(GSwatTab *tab)
{
	GeditDocument *doc;
	gchar *tip;
	gchar *uri;
	...
	{
		case GSWAT_TAB_STATE_LOADING_ERROR:
			tip = g_strdup_printf (_("Error opening file %s"),
					       ruri_markup);
			break;

		case GSWAT_TAB_STATE_REVERTING_ERROR:
			tip = g_strdup_printf (_("Error reverting file %s"),
					       ruri_markup);
			break;			

		case GSWAT_TAB_STATE_SAVING_ERROR:
...
GdkPixbuf *
_gswat_tab_get_icon (GSwatTab *tab)
{
	GdkPixbuf *pixbuf;
	GtkIconTheme *theme;
	GdkScreen *screen;
	...
	switch (tab->priv->state)
	{
		case GSWAT_TAB_STATE_LOADING:
			pixbuf = get_stock_icon (theme, 
						 GTK_STOCK_OPEN, 
						 icon_size);
			break;
		case GSWAT_TAB_STATE_REVERTING:
			pixbuf = get_stock_icon (theme, 
						 GTK_STOCK_REVERT_TO_SAVED, 
						 icon_size);						 
			break;
		case GSWAT_TAB_STATE_SAVING:
			pixbuf = get_stock_icon (theme, 
						 GTK_STOCK_SAVE, 
						 icon_size);
			break;
		case GSWAT_TAB_STATE_PRINTING:
			pixbuf = get_stock_icon (theme, 
						 GTK_STOCK_PRINT, 
						 icon_size);
			break;
		case GSWAT_TAB_STATE_PRINT_PREVIEWING:
		case GSWAT_TAB_STATE_SHOWING_PRINT_PREVIEW:		
			pixbuf = get_stock_icon (theme, 
		...
		case GSWAT_TAB_STATE_REVERTING_ERROR:		
		case GSWAT_TAB_STATE_SAVING_ERROR:
		case GSWAT_TAB_STATE_GENERIC_ERROR:
			pixbuf = get_stock_icon (theme, 
...
#if 0
GSwatTab *
gswat_tab_get_from_document (GeditDocument *doc)
{
	gpointer res;
	
	g_return_val_if_fail (GEDIT_IS_DOCUMENT (doc), NULL);
	
	res = g_object_get_data (G_OBJECT (doc), GSWAT_TAB_KEY);
	
	return (res != NULL) ? GSWAT_TAB (res) : NULL;
}
#endif

void
_gswat_tab_load (GSwatTab            *tab,
		 const gchar         *uri,
		 const GeditEncoding *encoding,
		 gint                 line_pos,
		 gboolean             create)
{
	...
	g_return_if_fail (GSWAT_IS_TAB (tab));
	g_return_if_fail (tab->priv->state == GSWAT_TAB_STATE_NORMAL);
...
void
_gswat_tab_revert (GSwatTab *tab)
{
	GeditDocument *doc;
	gchar *uri;
	...
	gswat_tab_set_state (tab, GSWAT_TAB_STATE_REVERTING);
...
static gboolean
gswat_tab_auto_save (GSwatTab *tab)
{
	GeditDocument *doc;
	...
	if ((tab->priv->state != GSWAT_TAB_STATE_NORMAL) &&
	    (tab->priv->state != GSWAT_TAB_STATE_SHOWING_PRINT_PREVIEW))
	{
...
void
_gswat_tab_save_as (GSwatTab            *tab,
		    const gchar         *uri,
		    const GeditEncoding *encoding)
{
	GeditDocument *doc;
			 ...
			 GSwatTab  *tab)
{
	...
	if (tab->priv->state == GSWAT_TAB_STATE_SHOWING_PRINT_PREVIEW)
		gswat_tab_set_state (tab, GSWAT_TAB_STATE_NORMAL);
	else
		gtk_widget_show (tab->priv->view_scrolled_window);		
}
...
static void
set_print_preview (GSwatTab  *tab, GtkWidget *print_preview)
{
...
static void
print_page_cb (GtkSourcePrintJob *pjob, GSwatTab *tab)
{
	gchar *str;
	gint page_num;
	gint total;
...
static void
preview_finished_cb (GtkSourcePrintJob *pjob, GSwatTab *tab)
{
	GnomePrintJob *gjob;
...
static void
print_finished_cb (GtkSourcePrintJob *pjob, GSwatTab *tab)
{
	GnomePrintJob *gjob;
...
static void
show_printing_message_area (GSwatTab      *tab,
			    gboolean       preview)
{
	GtkWidget *area;
		  ...
		  GtkTextIter   *end)
{
	...
	g_return_if_fail (doc != NULL);
	g_return_if_fail (gswat_tab_get_document (tab) == doc);
	...
	gswat_tab_set_state (tab, GSWAT_TAB_STATE_PRINTING);
...
void
_gswat_tab_print_preview (GSwatTab      *tab,
			  GeditPrintJob *pjob,
			  GtkTextIter   *start, 
			  GtkTextIter   *end)		  
{
	...
	gswat_tab_set_state (tab, GSWAT_TAB_STATE_PRINT_PREVIEWING);
...
void 
_gswat_tab_mark_for_closing (GSwatTab *tab)
{
	g_return_if_fail (GSWAT_IS_TAB (tab));
	g_return_if_fail (tab->priv->state == GSWAT_TAB_STATE_NORMAL);
	
	gswat_tab_set_state (tab, GSWAT_TAB_STATE_CLOSING);
}

gboolean
_gswat_tab_can_close (GSwatTab *tab)
{
	GeditDocument *doc;
	GSwatTabState  ts;

	g_return_val_if_fail (GSWAT_IS_TAB (tab), FALSE);

	ts = gswat_tab_get_state (tab);

	/* if we are loading or reverting, the tab can be closed */
	if ((ts == GSWAT_TAB_STATE_LOADING)       ||
	    (ts == GSWAT_TAB_STATE_LOADING_ERROR) ||
	    (ts == GSWAT_TAB_STATE_REVERTING)     ||
	    (ts == GSWAT_TAB_STATE_REVERTING_ERROR)) /* CHECK: I'm not sure this is the right behavior for REVERTING ERROR */
		return TRUE;

	/* Do not close tab with saving errors */
	if (ts == GSWAT_TAB_STATE_SAVING_ERROR)
		return FALSE;
...
/**
 * gswat_tab_get_auto_save_enabled:
 * @tab: 
 * 
...
#if 0
gboolean
gswat_tab_get_auto_save_enabled	(GSwatTab *tab)
{
	gedit_debug (DEBUG_TAB);
...
/**
 * gswat_tab_set_auto_save_enabled:
 * @tab: a #GSwatTab
...
#if 0
void
gswat_tab_set_auto_save_enabled	(GSwatTab *tab, 
				 gboolean enable)
{
	GeditDocument *doc = NULL;
	...
	tab->priv->auto_save = enable;
 	...
 	{
 		if ((tab->priv->state != GSWAT_TAB_STATE_LOADING) &&
		    (tab->priv->state != GSWAT_TAB_STATE_SAVING) &&
		    (tab->priv->state != GSWAT_TAB_STATE_REVERTING) &&
		    (tab->priv->state != GSWAT_TAB_STATE_LOADING_ERROR) &&
		    (tab->priv->state != GSWAT_TAB_STATE_SAVING_ERROR) &&
		    (tab->priv->state != GSWAT_TAB_STATE_REVERTING_ERROR))
		{
			install_auto_save_timeout (tab);
		}
...
/**
 * gswat_tab_get_auto_save_interval:
 * @tab: 
 * 
...
#if 0
gint 
gswat_tab_get_auto_save_interval (GSwatTab *tab)
{
	gedit_debug (DEBUG_TAB);

	g_return_val_if_fail (GSWAT_IS_TAB (tab), 0);
...
/**
 * gswat_tab_set_auto_save_interval:
 * @tab: a #GSwatTab
...
#if 0
void 
gswat_tab_set_auto_save_interval (GSwatTab *tab, 
				  gint interval)
{
	GeditDocument *doc = NULL;
