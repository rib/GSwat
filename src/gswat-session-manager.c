...
/* Macros and defines */
#define GSWAT_SESSION_MANAGER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), GSWAT_TYPE_SESSION_MANAGER, GSwatSessionManagerPrivate))
...
GType
gswat_session_manager_get_type(void) /* Typechecking */
{
	...
	{
	    sizeof(GSwatSessionManagerClass), /* class structure size */
	    NULL, /* base class initializer */
	    NULL, /* base class finalizer */
	    (GClassInitFunc)gswat_session_manager_class_init, /* class initializer */
	    NULL, /* class finalizer */
	    NULL, /* class data */
	    sizeof(GSwatSessionManager), /* instance structure size */
	    0, /* preallocated instances */
	    (GInstanceInitFunc)gswat_session_manager_init, /* instance initializer */
	    NULL /* function table */
	};

	/* add the type of your parent class here */
	self_type = g_type_register_static(G_TYPE_OBJECT, /* parent GType */
					   "GSwatSessionManager", /* type name */
	...
	{
	    (GInterfaceInitFunc)
		gswat_session_manager_mydoable_interface_init,
...
{   
    ...
    gswat_session_manager_signals[SIGNAL_NAME] =
	g_signal_new("signal_name", /* name */
		     G_TYPE_FROM_CLASS(klass), /* interface GType */
		     G_SIGNAL_RUN_LAST, /* signal flags */
		     G_STRUCT_OFFSET(GSwatSessionManagerClass, signal_member),
		     NULL, /* accumulator */
...
#if 0 /* template code */
	case PROP_NAME:
	    gswat_session_manager_set_property(self, g_value_get_int(value));
	    gswat_session_manager_set_property(self, g_value_get_uint(value));
	    gswat_session_manager_set_property(self, g_value_get_boolean(value));
	    gswat_session_manager_set_property(self, g_value_get_string(value));
	    gswat_session_manager_set_property(self, g_value_get_object(value));
	    gswat_session_manager_set_property(self, g_value_get_pointer(value));
	    break;
#endif
	default:
...
#if 0
static void
gswat_session_manager_mydoable_interface_init(gpointer interface,
					      gpointer data)
{
    MyDoableIface *mydoable = interface;
    g_assert(G_TYPE_FROM_INTERFACE(mydoable) == MY_TYPE_MYDOABLE);

    mydoable->method1 = gswat_session_manager_method1;
    mydoable->method2 = gswat_session_manager_method2;
}
#endif
...
/**
 * gswat_session_manager_get_PROPERTY:
 * @self:  A GSwatSessionManager.
 *
 * Fetches the PROPERTY of the GSwatSessionManager. FIXME, add more info!
 *
 * Returns: The value of PROPERTY. FIXME, add more info!
 */
PropType
gswat_session_manager_get_PROPERTY(GSwatSessionManager *self)
{
    g_return_val_if_fail(GSWAT_GSWAT_SESSION_MANAGER(self), /* FIXME */);
...
/**
 * gswat_session_manager_set_PROPERTY:
 * @self:  A GSwatSessionManager.
 ...
 */
void
gswat_session_manager_set_PROPERTY(GSwatSessionManager *self, PropType PROPERTY)
{
    g_return_if_fail(GSWAT_GSWAT_SESSION_MANAGER(self));
...
parse_session(xmlDocPtr doc, xmlNodePtr cur)
{
	...
	if(xmlStrcmp(cur->name, (const xmlChar *)"access_time") == 0)
	{
	    ...
	    gswat_session_set_access_time(session, strtol((char *)access_time, NULL, 10));
	    xmlFree(access_time); 
...
save_session(xmlNodePtr parent, GSwatSession *session)
{
    ...
    access_time_node = xmlNewChild(session_node, NULL, (const xmlChar *)"access_time", NULL);
