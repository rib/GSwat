...
#include "gswat-ptrace-stub.h"

static void spawn(gchar *read_fifo_name, gchar *write_fifo_name);
...
static void
spawn(gchar *read_fifo_name, gchar *write_fifo_name)
{
    int read_fifo_fd;
    int write_fifo_fd;
    ...
    /* FIXME - we don't need g_io_channels here, it's silly! */
    ...
    fprintf(stderr, "written=%d fifo_data->len=%d\n",
	    written,
	    fifo_data->len);
